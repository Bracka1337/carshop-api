        ...
        <div class="flex items-center gap-4">
            ...
            @foreach ($elements as $element)
                ...
                @if (is_string($element))
                    <span
                        class="relative h-10 max-h-[40px] w-10 max-w-[40px] select-none rounded-full text-center align-middle font-sans text-xs font-medium uppercase text-indigo-600 transition-all disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none">{{ $element }}</span>
                @endif
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        ...
                        @elseif ($page == 1 || $page == $paginator->lastPage() || ($page >= $paginator->currentPage() - 3 && $page <= $paginator->currentPage() + 3))
                            <a href="{{ $url }}"
                                class="relative h-10 max-h-[40px] w-10 max-w-[40px] select-none rounded-full text-center align-middle font-sans text-xs font-medium uppercase text-indigo-600 transition-all hover:bg-indigo-600/10 active:bg-indigo-600/20"
