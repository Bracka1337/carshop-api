            ...
            @if (is_array($element))
                @foreach ($element as $page => $url)
                    ...
                    @elseif ($page == $currentPage - 2 || $page == $currentPage + 2)
                        <span class="relative h-8 sm:h-10 max-h-[40px] w-8 sm:w-10 max-w-[40px] select-none rounded-full text-center align-middle font-sans text-xs font-medium uppercase text-indigo-600 transition-all disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none">...</span>
                    @endif
                @endforeach
            @endif
        @endforeach
