<div id="modal" class="fixed top-0 left-0 right-0 bottom-0 bg-black bg-opacity-50 z-30 hidden">
    <div class="mt-10 max-w-screen-2xl mx-auto px-4 sm:px-4 lg:px-5 bg-white shadow-lg rounded-xl hidden h-5/6 flex flex-col" id="productDetail" product-data-id="{{ $product->id }}">
        ...
        <div class="relative flex justify-end h-5 z-50">
            <span id="closeModal" class="cursor-pointer text-gray-500 hover:text-gray-800 text-3xl">&times;</span>
        </div>
        {{-- product details --}}
        <div class="flex flex-grow relative h-full">
            <div class="sm:w-full md:w-1/2 lg:w-2/5 xl:w-1/2 flex items-center justify-center rounded-lg bg-red-300 mb-5 relative">
                <!-- product image -->
                <div class="relative w-full h-full" data-carousel-inner>
                    <!-- Item 1 -->
                    @foreach ($product->images as $key => $image)
                        <div class="absolute inset-0 duration-700 ease-in-out h-full" data-carousel-item>
            ...
            <div class="sm:w-full md:w-1/2 lg:w-3/5 xl:w-1/2 px-4 flex flex-col justify-between">
                <div>
                    ...
                    <div>
                        <table class="w-full text-base text-left rtl:text-right text-gray-500">
                            <thead class="text-base text-gray-700">
                                <tr>
                                    <th scope="col" class="px-6 py-2 border-b border-gray-200">Type</th>
                                    <td class="px-6 py-2 border-b border-gray-200 font-bold text-right">{{ $product->type }}</td>
                                </tr>
                                <tr>
                                    <th scope="col" class="px-6 py-2 border-b border-gray-200">Size</th>
                                    <td class="px-6 py-2 border-b border-gray-200 font-bold text-right">{{ $product->diameter }}</td>
                                </tr>
                                <tr>
                                    <th scope="col" class="px-6 py-2 border-b border-gray-200">Width</th>
                                    <td class="px-6 py-2 border-b border-gray-200 font-bold text-right">{{ $product->width }}</td>
                                </tr>
                                <tr>
                                    <th scope="col" class="px-6 py-2 border-b border-gray-200">Et</th>
                                    <td class="px-6 py-2 border-b border-gray-200 font-bold text-right">{{ $product->et }}</td>
                                </tr>
                                <tr>
                                    <th scope="col" class="px-6 py-2 border-b border-gray-200">Cb</th>
                                    <td class="px-6 py-2 border-b border-gray-200 font-bold text-right">{{ $product->cb }}</td>
                                </tr>
                                <tr>
                                    <th scope="col" class="px-6 py-2 border-b border-gray-200">Bolt</th>
                                    <td class="px-6 py-2 border-b border-gray-200 font-bold text-right">{{ $product->bolt }}</td>
                                </tr>
                                <tr>
                                    <th scope="col" class="px-6 py-2 border-b border-gray-200">Bolt Diameter</th>
                                    <td class="px-6 py-2 border-b border-gray-200 font-bold text-right">{{ $product->bolt_diameter }}</td>
                                </tr>
                            </thead>
                        </table>
                    </div>
